...
# Restart experiments with beta1=0.9 only
# This script runs experiments across GPUs 1, 2, and 3
...
# Hyperparameter values to sweep - only beta1=0.9
BETA1_VALUES=(0.9)
...
# Generate all combinations upfront (same as original script)
...
# Function to start a tmux session and run experiments (same as original script)
run_experiments() {
    local gpu_id="$1"
    shift
    local experiments=("$@")
...
# Distribute experiments across GPUs 1, 2, 3
...
# Main execution
echo "=== Running beta1=0.9 Experiments on GPUs 1, 2, 3 ==="
...
echo ""
echo "To monitor GPU sessions:"
echo "  tmux attach-session -t gpu1_batch_optimizer"
echo "  tmux attach-session -t gpu2_batch_optimizer"
echo "  tmux attach-session -t gpu3_batch_optimizer"

echo ""
echo "To kill sessions:"
echo "  tmux kill-session -t gpu1_batch_optimizer"
echo "  tmux kill-session -t gpu2_batch_optimizer"
echo "  tmux kill-session -t gpu3_batch_optimizer"
