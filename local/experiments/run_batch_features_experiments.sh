...
# CrossLayer Transcoder Batch Features Experiment Runner
# This script creates tmux sessions for each GPU and runs experiments with different numbers of features
...
NUM_FEATURES_VALUES=(1000 2000 4000 6000 8000 10000 12000)

# Generate all combinations upfront (in this case, just the features values)
generate_all_combinations() {
    local combinations=()
    
    for num_features in "${NUM_FEATURES_VALUES[@]}"; do
        combinations+=("${num_features}")
    done
...
run_experiments() {
    ...
    local session_name="gpu${gpu_id}_batch_features"
    
    echo "Starting batch Features experiments for GPU $gpu_id"
    ...
    for experiment in "${experiments[@]}"; do
        ...
        # Add number of features override (for both main model and dead features)
        cmd="$cmd --model.init_args.model.init_args.d_features $num_features"
        cmd="$cmd --model.init_args.dead_features.init_args.n_features $num_features"
        
        echo "  Adding to sequence: GPU $gpu_id, num_features=$num_features"
...
# Main execution
echo "=== CrossLayer Transcoder Batch Features Experiment Runner ==="
echo "Base config: $BASE_CONFIG"
echo "Testing hyperparameters:"
echo "  - num_features: [${NUM_FEATURES_VALUES[*]}]"
echo ""
...
tmux list-sessions | grep batch_features || echo "No batch Features sessions found"

echo ""
echo "To monitor a specific GPU session:"
echo "  tmux attach-session -t gpu1_batch_features" 
echo "  tmux attach-session -t gpu2_batch_features"
echo "  tmux attach-session -t gpu3_batch_features"
...
total_combinations=${#NUM_FEATURES_VALUES[@]}
echo ""
