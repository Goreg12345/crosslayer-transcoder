...
def benchmark_loader(loader, mode_name: str, duration: float = 30.0) -> float:
    ...
    samples_per_second = (
        (batch_count * batch_size) / total_time if total_time > 0 else 0
    )
...
def benchmark_datamodule_shared_memory(
    batch_size: int = 5000, duration: float = 30.0
) -> Optional[float]:
    """
    ...
    try:
        ...
        print(
            f"   Estimated memory usage: {datamodule.get_memory_estimate_gb():.2f} GB"
        )
...
def benchmark_datamodule_simple_buffer(
    batch_size: int = 5000, duration: float = 30.0
) -> Optional[float]:
    """
        ...
        if len(results) > 1:
            ...
            if shared_result and simple_result:
                if shared_result > simple_result:
                    ...
                    print(
                        f"   Shared memory is {improvement:+.1f}% faster than simple buffer"
                    )
                else:
                    improvement = (simple_result - shared_result) / shared_result * 100
                    print(
                        f"   Simple buffer is {improvement:+.1f}% faster than shared memory"
                    )
    else:
