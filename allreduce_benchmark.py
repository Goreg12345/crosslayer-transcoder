...
def create_model_gradients():
    ...
    tensors = {
        'W_dec': torch.randn(12, 12, 6144, 768, device=device, dtype=torch.float32),    # 2592 MB
        'W_enc': torch.randn(12, 768, 6144, device=device, dtype=torch.float32),       # 216 MB  
...
def benchmark_allreduce(tensors, num_iterations=20):
    ...
    if rank == 0:
        ...
        print(f"Benchmarking AllReduce with {len(tensors)} tensors, {total_mb:.1f} MB total")
    ...
    for name, tensor in tensors.items():
        ...
        if rank == 0:
            ...
            print(f"{name}: {size_mb:.1f} MB → {avg_time*1000:.1f}±{std_time:.1f} ms ({bandwidth_gbps:.1f} GB/s)")
...
def test_tensor_sizes():
    ...
    for size_mb in test_sizes_mb:
        ...
        if rank == 0:
            print(f"{size_mb:4d} MB: {avg_time*1000:6.1f} ms ({bandwidth_gbps:5.1f} GB/s)")
...
def main():
    ...
    if rank == 0:
        ...
        print(f"Your model gradients ({total_mb:.1f} MB): {total_time*1000:.1f} ms AllReduce")
        print(f"This allows {1/total_time:.2f} training iterations per second (communication only)")
        ...
        print(f"Communication overhead: {total_time*1000/(1000/1.4)*100:.1f}% of total iteration time")
