...
def dead_features_neuron_indices(metric_params):
    ...
    n_features, n_layers = metric_params
    return DeadFeatures(n_features=n_features, n_layers=n_layers, return_neuron_indices=True)
...
class TestDeadFeaturesIntermediate:
    ...
    def test_handpicked_intermediate_values(self):
        ...
        metric = DeadFeatures(n_features=n_features, n_layers=n_layers, return_per_layer=True)
...
class TestDeadFeaturesNeuronIndices:
    ...
    def test_neuron_indices_basic(self):
        ...
        metric = DeadFeatures(n_features=n_features, n_layers=n_layers, return_neuron_indices=True)
        ...
        assert torch.equal(torch.sort(result["layer_indices"])[0], torch.sort(expected_layer_indices)[0])
        assert torch.equal(torch.sort(result["feature_indices"])[0], torch.sort(expected_feature_indices)[0])
...
class TestDeadFeaturesEdgeCases:
    ...
    def test_large_batch_consistency(self):
        ...
        metric1 = DeadFeatures(n_features=n_features, n_layers=n_layers, return_per_layer=True)
        metric2 = DeadFeatures(n_features=n_features, n_layers=n_layers, return_per_layer=True)
...
class TestDeadFeaturesLogFreqs:
    ...
    def test_log_freqs_basic(self):
        ...
        n_features, n_layers = 4, 2
        metric = DeadFeatures(n_features=n_features, n_layers=n_layers, return_log_freqs=True)
        ...
        expected_log_freqs = torch.tensor(
            [
                [
                    torch.log10(torch.tensor(3 / 6)),
                    torch.log10(torch.tensor(1 / 6)),
                    -10,
                    torch.log10(torch.tensor(2 / 6)),
                ],
                [
                    torch.log10(torch.tensor(2 / 6)),
                    -10,
